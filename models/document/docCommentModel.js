...
const tableName = 'tbl_doc_comment';
...
module.exports = {
    ...
    create: async(payload) => {
        try {
            const { id_doc, content, id_user, id_parent, id_user_tag, user_name_tag, img, created_time } = payload;
            const query = `insert into ${tableName} (id_doc, content, id_user, id_parent, id_user_tag, img, user_name_tag, created_time) values (?, ?, ?, ?, ?, ?, ?, ?)`;
            return await helpers.promisify(cb => database.query(query, [id_doc, content, id_user, id_parent, id_user_tag, img, user_name_tag, created_time], cb));
        } catch(e) {
