...
const tableName = 'tbl_doc_comment_like';
...
module.exports = {
    ...
    create: async(payload) => {
        try {
            const { id_comment, id_doc , id_user, created_time} = payload;
            const query = `insert into ${tableName} (id_comment, id_doc, id_user, created_time) values (?, ?, ?, ?)`;
            return await helpers.promisify(cb => database.query(query, [id_comment, id_doc, id_user, created_time], cb));
        } catch(e) {
